...
test("painel de detalhes: abre, navega, fecha e atualiza o link", async ({ page }) => {
  ...
  await drawer.getByRole("button", { name: /Planogramas com QR Code →/ }).click();
  await expect(drawer.locator("h2")).toHaveText("Planogramas com QR Code");
  await expect(page).toHaveURL(/#projeto\/planograma$/);
